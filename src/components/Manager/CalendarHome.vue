  ...
  <div class="h-screen">
    ...
    >
      <h1 class="text-2xl font-bold">
        <span class="mr-4">{{ currentDateThai }}</span>
      </h1>
        ...
        <ul class="events">
          ...
          >
            <a-badge
              :status="item.type"
              :text="item.content"
              class="text-black font-bold py-2 rounded-md px-2 mt-2"
              :class="{
          ...
          <li
            v-for="item in getListprojectData(current)"
            :key="item.content"
            @click="showProjectDetails(item)"
          >
            <a-badge
              :text="`กำหนดส่งโปรเจค (${item.content}) `"
  ...
  methods: {
    ...
    async fetchEventData() {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_VUE_APP_PROJECT}/event/getevent`,
          {
            headers: {
    ...
    async fetchProjects() {
      try {
        this.isLoading = true;
        const response = await axios.get(
          `${import.meta.env.VITE_VUE_APP_PROJECT}/requset/getall`,
          {
            headers: {
              "auth-token": localStorage.getItem("token"),
            },
