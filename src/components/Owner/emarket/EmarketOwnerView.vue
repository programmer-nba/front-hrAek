        ...
        <div class="sm:flex sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-x-3">
                    <h2 class="text-lg font-medium text-gray-800 ">ข้อมูลคำร้องขอฝากขายสินค้า</h2>
                ...
                <tbody class="bg-white">
                    <tr v-for="product in paginatedItems" :key="product.id" class="text-[#000000]">
                        ...
                        <td class="px-2 py-3 text-sm border border-[#0C359E] text-center whitespace-nowrap">
                            <div :class="{
                                'inline px-3 py-1 text-sm font-normal rounded-full text-emerald-500 gap-x-2 bg-emerald-100/60 ': product?.request_status_detail[product.request_status_detail.length - 1]?.status == 'อนุมัติ',
                                'inline px-3 py-1 text-sm font-normal rounded-full text-orange-500 gap-x-2 bg-orange-100/60 ': product?.request_status_detail[product.request_status_detail.length - 1]?.status === 'รอการอนุมัติ',
                                'inline px-3 py-1 text-sm font-normal rounded-full text-red-500 gap-x-2 bg-red-100/60 ': product?.request_status_detail[product.request_status_detail.length - 1]?.status === 'ไม่อนุมัติ',
                            }">
                                {{ product?.request_status_detail[product.request_status_detail.length - 1]?.status }}
                            </div>
                        </td>
                        <td class="px-2 py-3 text-sm border border-[#0C359E] text-center whitespace-nowrap">
                        ...
                        <td class="px-4 py-3 text-sm border border-[#0c359e] text-center">
                            <EmarketDetail :product_id="product._id" :product="product" />
                        </td>
                        <td class="px-4 py-3 text-sm border border-[#0C359E]">

                            <div class="space-x-2  py-4 text-center">
                                <button @click="cancelproduct(product._id)"
                                    v-if="product?.request_status_detail[product.request_status_detail.length - 1]?.status === 'รอการอนุมัติ'"
                                ...
                                <button @click="confirmproduct(product._id)"
                                    v-if="product?.request_status_detail[product.request_status_detail.length - 1]?.status === 'รอการอนุมัติ'"
...
import EmarketDetail from "./DetailEmarket.vue";
...
export default {
    ...
    methods: {
        ...
        showItemsByPosition(status_appover) {
            ...
            this.filteredItems = this.requestproduct.filter(item => item.request_status_detail[item.request_status_detail.length - 1].status === status_appover);
            this.currentPage = 1; // กลับไปที่หน้าแรก
        },
        nextPage() {
        ...
        async fetchrequestproduct() {
            try {
                const response = await axios.get(`${import.meta.env.VITE_VUE_APP_DECCAN}/partners/request/product`,
                    {
                        headers: {
                            "auth-token": localStorage.getItem("token"),
                        },
                    });
                console.log("requestproduct", response.data.data);
        ...
        async confirmproduct(productId) {
            try {
                const response = await axios.put(
                    `${import.meta.env.VITE_VUE_APP_DECCAN}/partners/requestproduct/approve/${productId}`, {},
                    {
                        headers: {
                            "auth-token": localStorage.getItem("token"),
                        },
        ...
        async cancelproduct(productId) {
            try {
                const response = await axios.put(
                    `${import.meta.env.VITE_VUE_APP_DECCAN}/partners/requestproduct/unapprove/${productId}`, {},
                    {
                        headers: {
                            "auth-token": localStorage.getItem("token"),
                        },
