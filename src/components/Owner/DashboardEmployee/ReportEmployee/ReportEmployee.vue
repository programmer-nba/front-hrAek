      ...
      <div class="w-full px-3 mb-6 mx-auto">
        ...
        <div class="mx-10 py-2 space-y-2">
            <div class="flex sm:justify-start justify-center">
              ...
              <div class="mx-10 py-2 space-y-2">
                <input type="file" @change="handleFileChange($event, 'file1')" required>
              </div>
              ...
              <div class="mx-10 py-2 space-y-2">
                <input type="file" @change="handleFileChange($event, 'file2')" required>
              </div>
              ...
              <div class="mx-10 py-2 space-y-2">
                <input type="file" @change="handleFileChange($event, 'file3')" required >
              </div>
              ...
              <div class="mx-10 py-2 space-y-2">
                <input type="file" @change="handleFileChange($event, 'file4')"  required>
              </div>
...
export default {
 ...
 methods: {
    // ฟังก์ชันสำหรับการจัดการไฟล์ที่เลือก
    handleFileChange(event, fileKey) {
      this.files[fileKey] = event.target.files[0];
    },
    
    // ฟังก์ชันตรวจสอบว่าไฟล์ทั้งหมดถูกเลือกหรือไม่
    checkInputs() {
      if (!this.files.file1 || !this.files.file2 || !this.files.file3 || !this.files.file4) {
        this.errorMessage = 'กรุณาเลือกไฟล์ให้ครบทั้ง 4 ไฟล์';
      } else {
        this.errorMessage = '';
        // ที่นี่คุณสามารถดำเนินการเพิ่มเติมกับไฟล์ที่เลือก เช่น ส่งไปยังเซิร์ฟเวอร์
        alert('ไฟล์ทั้งหมดได้รับการเลือกแล้ว');
      }
    }
  },
