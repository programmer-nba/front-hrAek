  ...
  <section class="bg-[#EAFDFC] p-3 sm:p-5 antialiased h-screen">
    <div class="mx-auto max-w-screen-2xl px-4 lg:px-12">
      <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
        ...
        >
          <h1>
            <span class="text-[#0002A1] font-bold text-[20px] sm:text-3xl"
                  ...
                  <div
                    :class="{
                      ...
                      'bg-yellow-200 text-yellow-900': [
                        ...
                        'รอผู้จัดการอนุมัติ',
                        'รอผู้บริหารอนุมัติ',
                      ].includes(document.status_document),
...
export default {
      ...
      groupedfilter: [
        ...
        },
        {
          ...
          items: [
            ...
            {
              label: "รอหัวหน้าแผนกอนุมัติ",
              status_document: "รอหัวหน้าแผนกอนุมัติ",
            },
            {
              label: "รอผู้จัดการอนุมัติ",
              status_document: "รอผู้จัดการอนุมัติ",
            },
            {
      ...
      <div class="grid grid-cols-3 mt-5">
        ...
        <div class="flex justify-center items-center">
          <p class="text-red-500">( 
            ${this.getRole(documents.status_detail[0].role)} -  
          ${this.getPosition(documents.status_detail[0].position)} )</p>
        </div>
      </div>
    </head>
    <body>
  ...
  `;
    ...
    `;
      ...
      if (documents.status_detail[0].status === "อนุมัติ") {
        content += `
      <div class="flex flex-col gap-2 mt-6">
              <div class="flex">
                    ลงชื่อ <div class="ml-3">${this.getEmployeeName(
                      documents.status_detail[1].employee_id
                    )}</div>
              </div>
              <div class="flex justify-center">
                    (${this.getEmployeeFullName(
              ...
              <div class="flex justify-center">
                    ${this.getPosition(documents.status_detail[1].position)}
              </div>
              <div class="flex">
                    วันที่ ${this.formatDate(documents.status_detail[1].date)}
              </div>
          </div>
        </div>
        </div>
        <div class="flex justify-around  mt-2">
          <div class="flex flex-col gap-2 mt-6">
              <div class="flex">
                    ลงชื่อ <div class="ml-3">${this.getEmployeeName(
                      documents.status_detail[2].employee_id
                    )}</div>
              </div>
              <div class="flex justify-center">
                    (${this.getEmployeeFullName(
              ...
              <div class="flex justify-center">
                    ${this.getPosition(documents.status_detail[2].position)}
              </div>
              <div class="flex">
                    วันที่ ${this.formatDate(documents.status_detail[2].date)}
              </div>
          </div>
          <div class="flex flex-col gap-2 mt-6">
               <div class="flex">
                    ลงชื่อ <div class="ml-3">${this.getEmployeeName(
                      documents.status_detail[3].employee_id
                    )}</div>
              </div>
              <div class="flex justify-center">
                    (${this.getEmployeeFullName(
              ...
              <div class="flex justify-center">
                    ${this.getPosition(documents.status_detail[3].position)}
              </div>
              <div class="flex">
                    วันที่ ${this.formatDate(documents.status_detail[3].date)}
              </div>
          </div>
        </div>
    </div>
