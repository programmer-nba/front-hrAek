<template>
  <div>
    ...
    >
      <div class="grid grid-cols-2 gap-8">
        ...
        <div class="mb-4">
          ...
          <p>{{ detailEmployee.product_detail[0].packagename }}</p>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
          ...
          <ul>
            <li
              v-for="detailEmployee in detailEmployee.product_detail"
    ...
    >
      <form class="px-4 sm:px-10 mt-2 sm:mt-5">
        <div class="grid gap-3 sm:gap-6 mb-3 sm:mb-6 md:grid-cols-2">
          ...
          <div>
            <label
              for="editProjectPosition"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >ตำแหน่งงาน</label
            >
            <select
              v-model="editedProject.position"
              @change="fetchEmployeesForEditedProject"
              id="editProjectPosition"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"
              required
            >
          ...
          <div v-if="employeeDropdownVisible && employees.length">
            <label
              for="editProjectEmployee"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >พนักงาน</label
            >
            <select
              v-model="editedProject.employee_id"
              @change="setEmployeeEdit"
              id="editProjectEmployee"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"
              required
            >
          ...
          <div>
            <label
              for="editProjectDueDate"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >วันกำหนดส่ง</label
            >
            <Calendar
              v-model="editedProject.due_date"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2 sm:p-2.5"
              required
            />
          </div>
          ...
          <div>
            ...
            >
            <select
              v-model="editedProject.type_code"
              id="editProjectType"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"
              required
            >
          ...
          <div>
            <label
              for="editProjectSubType"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >ประเภทงาน</label
            >
            <input
              v-model="editedProject.sub_type"
              type="text"
          ...
          <div>
            <label
              for="editProjectCustomer"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >ชื่อลูกค้า</label
            >
            <input
              v-model="editedProject.customer_name"
              type="text"
          ...
          <div>
            <label
              for="editProjectStatus"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >สถานะ</label
            >
            <select
              v-model="editedProject.status.status_name"
          ...
          <div v-if="editedProject.status.status_name === 'โปรเจ็คเสร็จลุล่วง'">
            <label
              for="editProjectDueDate"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >วันที่โปรเจคสำเร็จ</label
            >
            <Calendar
              v-model="editedProject.finish_date"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2 sm:p-2.5"
              required
            />
          </div>
          ...
          <div class="col-span-2">
            <label
              for="editProjectDetail"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >รายละเอียดงาน</label
            >
            <textarea
              v-model="editedProject.detail"
          ...
          <div class="col-span-2">
            <label
              for="editProjectRemark"
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900"
              >หมายเหตุ</label
            >
            <textarea
              v-model="editedProject.remark"
              id="editProjectRemark"
              rows="3"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"
              placeholder=""
            ></textarea>
      ...
      <div class="w-full max-w-full px-3 mb-6 mx-auto">
        ...
        >
          <div class="shadow-xl">
            ...
            <div
              class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-4 ml-10 py-2"
            >
              <div>
                <button
              ...
              <div class="flex items-center mr-10 w-full">
                <label for="filterType" class="mr-3 text-gray-600"
                  >ประเภทงาน:</label
                >
                <select
                  v-model="filterType"
                ...
                <label for="filterStatus" class="ml-2 text-gray-600"
                  >สถานะ:</label
                >
                <select
                ...
                >
                  <thead class="align-bottom">
                    <tr class="font-semibold text-sm text-center text-black">
                      <th class="pb-3 text-center min-w-[100px]">รหัส</th>
                      <th class="pb-3 text-center min-w-[100px]">หัวข้อ</th>
                      <th class="pb-3 text-center min-w-[100px]">
                        <div class="flex">
                          <button
                            class="bg-white hover:text-purple-500 text-sm"
                            @click="seeName = !seeName"
                          >
                            พนักงาน
                          </button>
                        </div>
                  ...
                  <tbody>
                    ...
                    >
                      <td class="text-start">
                        <span
                          class="font-semibold pl-2 text-light-inverse text-md/normal"
                          >{{ project.code }}</span
                        >
                      </td>
                      <td class="p-3 pl-0">
                      ...
                      <td class="p-3 pr-0 text-center">
                        <div
                          ...
                          class="font-semibold text-light-inverse text-md/normal flex"
                        >
                          <div
                            v-for="em in getEmployeesData(project.employees)"
                            :key="em._id"
                          >
                            <div
                              v-if="!seeName"
                            ...
                            >
                              <Image
                                ...
                                :src="em.imageUrl"
                                :alt="em.nick_name"
                                imageClass="w-[45px] h-[45px] rounded-full shadow-md"
                                preview
                              />
                              <div
                                v-else
                                class="w-[45px] h-[45px] rounded-full bg-sky-200"
                              >
                                <small class="p-0 m-0">{{
                                  em.nick_name
                                }}</small>
                            ...
                            <div
                              v-else
                              class="flex items-center font-normal flex-wrap gap-2"
                            >
                          ...
                          <ButtonP
                            icon="pi pi-plus-circle text-purple-500 font-bold text-4xl"
                        ...
                        <div v-else>
                          <ButtonP
                            icon="pi pi-plus-circle text-purple-500 font-bold text-xl"
                        ...
                        <span
                          ...
                          :class="{
                            ...
                            'bg-red-500 text-white opacity-90':
                              project.status.status_name === 'โปรเจ็คถูกระงับ',
                          }"
                        >
                      ...
                      <td class="pr-0 text-center">
                        <div v-if="project.dueDate">
                          <p
                            class="font-semibold m-0 p-0 text-light-inverse text-md/normal"
                          >
                      ...
                      <td class="pr-0 text-center">
                        ...
                        <button
                          @click="confirmDelete(project._id)"
    ...
    <Dialog
      v-model:visible="openSelectEmployeesDialog"
      :modal="true"
      class="w-full md:w-5/6 lg:w-2/3 xl:w-1/2 min-h-[500px] bg-white"
      header=" "
    >
      <p class="mt-2">
        พนักงานที่เลือก
        <span
          class="rounded-full px-2.5 text-white"
          :class="selectedEmployees.length ? 'bg-red-500' : 'bg-gray-500'"
      ...
      <div class="flex justify-center border-b-4 border-dashed my-3">
        <SelectButton
          class="py-3"
          v-model="filterEmployees"
          :options="employTypesOptions"
          optionLabel="title"
          multiple
          aria-labelledby="multiple"
          :pt="{ button: 'border text-xs border-collapse' }"
        />
      </div>
      <div class="flex flex-wrap justify-start items-start gap-2">
        <div v-for="employ in filteredEmployees" :key="employ._id">
          <div
            class="flex justify-between w-[300px] border rounded shadow-sm py-2 px-3 items-center"
            :class="
              selectedEmployees.includes(employ) ? 'bg-green-100' : 'bg-white'
            "
          >
            <div class="flex items-center gap-x-2">
              <Image
                v-if="employ.imageUrl"
                :src="employ.imageUrl"
                :alt="employ.image"
                preview
                imageClass="w-[40px] h-[40px] rounded-full"
            ...
            <div class="flex items-center">
              <Checkbox
                ...
                name="employ"
                :value="employ"
                inputClass="border-2 border-green-500"
              />
            </div>
          </div>
        </div>
      ...
      <template #footer>
        <div class="h-[30px] my-3 flex flex-col w-full items-end gap-y-2">
          ...
          <small
            class="text-red-500"
            :class="selectedEmployees.length ? 'hidden' : ''"
            >กรุณาเลือกพนักงานอย่างน้อย 1 คน</small
          >
        </div>
      </template>
    </Dialog>
    ...
    <Dialog
      v-model:visible="openSelectDueDateDialog"
      :modal="true"
...
export default {
  ...
  data() {
    return {
      ...
      currentPage: 1,
      pageSize: 20, // กำหนดจำนวนรายการที่แสดงในหนึ่งหน้า
  ...
  computed: {
    filteredEmployees() {
      const customOrder = this.employTypesOptions.map((co) =>
        co.name.toLocaleLowerCase()
      );
      const result = this.filterEmployees.length
        ? this.employees
            .filter((em) =>
              this.filterEmployees.some((se) =>
                se.name.toLowerCase().includes(em.position.toLowerCase())
              )
            )
            .sort(
              (a, b) =>
    ...
    filteredProjects() {
      ...
      const filtered = this.projects.filter((project) => {
        const matchesSearch =
          project.code.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
  ...
  methods: {
    ...
    async submitEditedProject() {
      try {
        ...
        const response = await axios.put(
          `${import.meta.env.VITE_VUE_APP_DECCAN}/requset/update/project/${
    ...
    async submitProject() {
      ...
      payload.projectType = this.newProject.type?._id;
      try {
        this.isLoading = true;
