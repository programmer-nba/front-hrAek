  ...
  <div>
    ...
    <div class="relative min-h-screen flex">
      <div class="flex flex-col sm:flex-row items-center md:items-start sm:justify-center md:justify-start flex-auto min-w-0 bg-white">
        <div class="sm:w-1/2 xl:w-2/5 h-full hidden md:flex flex-auto items-center justify-start p-10 overflow-hidden bg-green-900 text-white bg-no-repeat bg-cover relative":style="{ 'background-image': `url(@/assets/character1.png)` }">
              ...
              <div class="space-y-5">
                <div class="space-y-2">
                  <label class="text-sm font-medium text-gray-700 tracking-wide"
                    >Username</label>
                  <input v-model="userid" @keyup.enter="login" class="w-full text-base px-4 py-2 border text-black border-gray-300 rounded-lg outline-[#0B60B0]" type="text" placeholder="กรอก username"/>
                </div>
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    <div class="relative">
                      <input v-model="password" @keyup.enter="login" :type="showPassword ? 'text' : 'password'" class="text-sm sm:text-base text-black px-4 rounded-lg border border-gray-300 w-full py-2 outline-[#0B60B0]" placeholder="กรอกรหัสผ่าน"/>
                      <i class="cursor-pointer text-gray-500 absolute right-4 top-1/2 -translate-y-1/2" :class="!showPassword ? 'pi pi-eye-slash' : 'pi pi-eye'" @click="showPassword = !showPassword">
                      </i>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <button @click="login" class="w-full flex items-center justify-center bg-gradient-to-r from-gray-700 to-gray-950 hover:bg-gradient-to-l hover:from-gray-600 hover:to-gray-900 text-gray-100 p-3 rounded-full tracking-wide font-semibold shadow-lg cursor-pointer transition ease-in hover:scale-105 duration-300">
...
<Dialog v-model:visible="isRegister" class="w-1/2"
      header="กรอกข้อมูลเพื่อสมัครสมาชิก">
      <form class="px-4 sm:px-10 mt-2 sm:mt-5">
      
        <div class="grid gap-3 sm:gap-6 mb-3 sm:mb-6 md:grid-cols-2">
           <div>
            <label for="role" class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*ตำแหน่งงานที่สมัคร</label>
            <Dropdown v-model="selectedSubPosition" :options="subForPosition" filter placeholder="เลือกตำแหน่ง" class="no-rounded w-full mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner">
          ...
          <div>
            <label for="role" class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*คำนำหน้า</label>
            <Dropdown v-model="selectedTitle" :options="subForTitle" filter placeholder="เลือกคำนำหน้า" class="no-rounded w-full mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner">
              <template #value="slotProps">
                <div v-if="slotProps.value" class="flex align-items-center">
         ...
         <div>
            <label 
              class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*ชื่อ</label>
          ...
          <div>
            <label class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*นามสกุล</label>
            <input  type="text" id="" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5" v-model="last_name"/>
          </div>

          <div>
            <label class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*เบอร์โทรศัพท์</label>
            <input  type="text" id="" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"  v-model="tel"/>
          </div> 
          ...
          <div>
            ...
            <input type="email" id=""class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"  v-model="email"/>
          </div>

          <div>
            <label class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*เลขบัตรประชาชน</label>
            <input  type="text" id=""class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5"  v-model="iden_number"/>
          </div>

          <div>
            <label for="datePicker"class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">เกิดวันที่:</label>
            <input type="date"  id="" v-model="birthday">
          </div>

          <div>
            <label class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*ที่อยู่ตามบัตรประชาชน</label>
            <input type="text" id="" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 sm:p-2.5" v-model="address" />
          </div> 
          ...
          <div >
          <label for="file-upload" class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*แนบสำเนาบัตรประชาชน</label>
            <input id="file-upload" type="file" @change="handleFileChange" @click="fileType = 'iden'"/>
          </div>

          <div>
          <label class="block mb-1 sm:mb-2 text-xs sm:text-base font-medium text-gray-900">*แนบหน้าสมุดบัญชีธนาคาร</label> 
        ...
        <div class="flex justify-end"> 
          <button  @click="SubmitRegister()" class="bg-blue-500 text-white p-2 rounded hover:bg-blue-700"> สมัครสมาชิก
          </button>
          <!-- type="button" -->
        </div> 
