<template>
    <!-- Modal body -->
    <div class="bg-[#E0F4FF] h-full pt-2">
    ...
    >
      ...
      <div>
        ...
        <p v-if="selectedDraft.type == 'OT'">
          <strong>ตั้งแต่เวลา</strong> {{ formatTime(selectedDraft.ot.timein) }}
          <strong>จนถึงเวลา</strong> {{ formatTime(selectedDraft.ot.timeout) }}
        </p>
        <div class="flex flex-col gap-2">
          <p class="text-center"><strong>รายละเอียด:</strong></p>
          <p class="overflow-y-scroll max-h-96" v-html="selectedDraft.detail"></p>
        </div>
        <!-- แสดงไฟล์ที่แนบมา -->
