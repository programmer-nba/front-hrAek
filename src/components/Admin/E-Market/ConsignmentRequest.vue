        ...
        <div class="sm:flex sm:items-center sm:justify-between mt-6">
            <div>
                <div class="flex items-center gap-x-3 mt-4">
                    <h2 class="text-lg font-medium text-gray-800 ">คำขอฝากขายสินค้า</h2>

                    <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full  ">{{ product.length }}
                        รายการ</span>
                </div>
                ...
                <tbody v-if="paginatedItems.length > 0" class="bg-white">
                    ...
                    <tr v-for="(item, index) in paginatedItems" :key="index" class="text-[#000000]">
                        ...
                        <td class="px-2 py-3 text-sm border border-[#0C359E] text-center">
                            <div>
                                <h2 class="font-medium text-gray-800">{{ item.product_partner_id?.partner_name }}</h2>
                                <h2 class="font-medium text-gray-800">{{ item.product_partner_id?.partner_company_name
                                    }}</h2>
                        ...
                        <td class="px-2 py-3 text-sm border text-center border-[#0C359E]">
                            <div>
                                <h4 class="text-black" v-html="item.product_detail"> </h4>

                                <span>({{ item.product_category }})</span>

                                <h4 class="text-black"> ราคา: {{ item.product_price }}</h4>
                                <h4 class="text-black"> จำนวนสต็อก: {{ item.product_stock }}</h4>
                        ...
                        <td class="px-4 py-3 text-sm border border-[#0C359E]">
                            <div class="flex justify-center gap-2">
                                ...
                                <button @click="approveProductRequest(item._id)" class="btn px-4 py-2"> อนุมัติ
                                </button>

                                <button @click="NotapproveProduct(item._id)"
...
export default {
    ...
    methods: {
        async NotapproveProduct(id) {
            try {
                await axios.put(`${import.meta.env.VITE_VUE_APP_DECCAN}/partners/requestproduct/unapprove/${id}`, null, {
                    headers: {
        ...
        async approveProductRequest(id) {
            try {
                await axios.put(`${import.meta.env.VITE_VUE_APP_DECCAN}/partners/requestproduct/approve/${id}`, null, {
                    headers: {
                        "auth-token": localStorage.getItem("token"),
                    },
                });
        ...
        showItemsByPosition(role) {
            ...
            this.filteredItems = this.product.filter(item => item.role === role);
            this.currentPage = 1; // กลับไปที่หน้าแรก
        },
        nextPage() {
        ...
        async getProduct() {
            try {
                ...
                this.product = response.data.data.reverse();
                this.filteredItems = this.product;
                console.log("คำร้องสินค้า", response.data.data);
